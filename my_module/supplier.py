...
class SupplierWindow:
    ...
    def create_treeview(self):
        ...
        self.trv.heading("2", text="Supplier ID")
    ...
    def setup_supplier_form(self):
        ...
        results = sql_query('SELECT NAME FROM Products', [])
        products = [result[0] for result in results]
        self.suppo = customtkinter.CTkOptionMenu(self.master, values=products)
    ...
    def setup_delete_supplier(self):
        customtkinter.CTkLabel(self.master, text="Delete a supplier: ", font=('Helvetica', 14, 'bold')).place(x=440,
                                                                                                              y=210)
        customtkinter.CTkLabel(self.master, text="Name:").place(x=460, y=250)
    ...
    def add_supplier(self):
        ...
        query = f"INSERT INTO Supplier VALUES ({placeholder},%s,%s)"
        parameters = product_list + variables
    ...
    def delete_supplier(self):
        id = []
        id.append(self.iden.get())
        sql_query('DELETE FROM Supplier WHERE Name=%s', id)
        self.fill_table()
    ...
    def fill_table(self):
        ...
        for row in rData:
            self.trv.insert("", 'end', iid=row[0], text=row[0], values=(row[0], row[1], row[2]))
